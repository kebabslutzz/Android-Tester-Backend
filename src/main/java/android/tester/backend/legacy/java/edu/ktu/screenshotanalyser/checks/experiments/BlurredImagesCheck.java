...
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.context.Drawable;
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.context.State;
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.utils.BlurDetector;
    ...
    if ((!backgroundElements.isEmpty()) && (backgroundElements.size() == elements.size())) {
      for (var element : backgroundElements) {
        ...
        if (!hasHighResolutionImage(images, control, 0.7f)) {
          ...
          result.addAnnotation(new DefectAnnotation(this, control.getBounds(), control.getBounds().width + "x" + control.getBounds().height + " -> " + maxImage.get()));
