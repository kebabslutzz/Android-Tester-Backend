package android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.context;

import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.checks.SystemContext;
...
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.tools.Settings;
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.utils.ImageUtils;
import android.tester.backend.legacy.java.edu.ktu.screenshotanalyser.utils.LazyObject;
...
public class AppContext {
  public AppContext(DataBase.Application app, File dataFolder, List<TestDevice> testDevices, SystemContext systemContext) throws IOException {
    this.app = app;
  ...
  public synchronized List<Drawable> findDrawable(Mat source) {
    ...
    if (mainColor.getLeft().equals(ImageUtils.zero)) {
...
*/
  ...
  private final LocalizedMessages messages = null;
  ...
  private final LazyObject<Drawable> smallestIcon = new LazyObject<>(() -> getAllImages().stream().filter(p -> p.isIcon() && p.getWidth() > 0).sorted((a, b) -> a.getWidth() - b.getWidth()).findFirst().orElse(null));
