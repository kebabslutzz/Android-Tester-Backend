...
public class ClippedControlCheck extends BaseTextRuleCheck implements IStateRuleChecker {
  ...
  private boolean isClipped(Control control, State state) {
    return (control.getBounds().x + control.getBounds().width > state.getImageSize().width + 5) || (control.getBounds().y + control.getBounds().height > state.getImageSize().height + 5);
  }
